...
# Converts and index (super act) into the relevant movement super action Enum, depending on the player
def index_to_move_super_act(action):
    return {
        0: index_to_move_action(6) + index_to_move_action(5) + index_to_move_action(4) +  index_to_attack_action(0) ,
        1: index_to_move_action(4) + index_to_move_action(5) + index_to_move_action(6) +  index_to_attack_action(0) ,
        ...
        if self.started:
            if not self.round_done and not self.stage_done and not self.game_done:
                ...
                if random.random() > 0.3:
                    ...
                    actions += index_to_super_action((move_action + attack_action)%6)
